...
# Class: OSU CS 493
# Term: Spring 2021
# Description: This API integrates Google Cloud Platform to provide
# an auth0 process that enables the user to view only the information
# They have access to after first signing in to provide authorization
#####################################################################
...
CLIENT_ID = 'X5EMLy60nTiOH4pk4EDUyz8y5XNuIPsf'
CLIENT_SECRET = '8sWrL161og9djh7S19QwYQBBr0YxlpyWZuPQbS44UDKnDdJV_drQZwZ7lXflLi5-'
DOMAIN = 'osu-493-morett.us.auth0.com'

CALLBACK_URL = 'https://final-api-morett.uc.r.appspot.com/callback'
...
# Needed before showing the userinfo page
def requires_auth(f):
   @wraps(f)
...
# Route to get all boats or add a boat
...
# Routes for Adding (PUT) and deleting (DELETE) a specfic load from a 
# specific boat
...
# Routes for creating a new load (POST) or retreiving the loads list (GET)
...
# Routes for retrieving (GET) and deleting (DELETE) a specific load
...
# The home route brings the user to the welcome page where they may
# choose to sign or create an account
@app.route('/')
def welcome_page():
   return render_template('welcomePage.html')


# The route to the user page once the user is authenticated
# Will show the user's JWT
@app.route('/userinfo')
@requires_auth
def user_page():
   jwt = session['JWT']
   ...
   return render_template('userInfoPage.html', JWT = jwt, USER_ID = user_id)       
...
# Logs user out, clears session data and brings user back to the welcome page
@app.route('/logout')
def logout():
   # Clear session stored data
   session.clear()
   # Redirect user to logout endpoint
   params = {'returnTo': url_for('welcome_page', _external=True), 'client_id': CLIENT_ID}
   return redirect(auth0.api_base_url + '/v2/logout?' + urlencode(params))  


####################### HELPER FUNCTIONS ##################################

# Add a user to the database when they log in using Auth0
def add_user_to_db(auth, name):
   new_user = datastore.entity.Entity(key=client.key(USERS))
   new_user.update({"name": name, "user_id": auth})
   client.put(new_user) 

# Helper function to create a boat object to include in response
...
# Helper function to create a user object to include in response
...
# Helper function to create a load object for response
...
# Helper function to create a boat_load object to store on a boat
def boat_load_object(load):
   boat_load = dict()
   boat_load['id'] = load.key.id
   # boat_load['self'] = "http://" + request.host + '/loads/' + str(load.key.id)
   return boat_load

# Helper function to create a load object for response in the loads list 
# for a specific
...
# Helper function to create a carrier object for a load
def load_carrier_object(boat): 
   carrier = dict()
   carrier['id'] = boat.key.id
   carrier['name'] = boat['name']
   # carrier['self'] = "http://" + request.host + '/boats/' + str(boat.key.id)
   return carrier

# Helper function to create error message responses
def error_response(message): 
   response = dict()
   response['Error'] = message
   return response

#Helper function to create a result with header and status code
...
# Helper function to get the self url with an id
